...
void Genetic::mutationScramble(Individuo &f)
{
	...
	do{
		random_vehicle1 = rand() % f.route.size();
	}while(f.route[random_vehicle1].size() < 4);
	
	begin = rand()% ( (f.route[random_vehicle1].size() - 2) - 1) + 1;

	end = rand()% ( (f.route[random_vehicle1].size() - 1) - (begin + 1) ) + (begin + 1);

	for(int i = 0; i < f.route[random_vehicle1].size(); i++){
		cout << f.route[random_vehicle1][i] << " ";
	}
	cout << endl;
